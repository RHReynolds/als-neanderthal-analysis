...
- As we are using top 0.5%, 1% and 2% these negative values do not affect the XP-EHH annotations (nor the closely related iHS, which is also based on EHH). **If we choose to run these annotations as continuous annotations, we should be careful to remove these SNPs from annotation.** 
...
- Using FDR-corrected results, a significant depletion was observed for:
    ...
    - PD $h^2_{SNP}$ using the top 0.5%, 1% and 2% XP-EHH statistic, suggesting that SNPs under positive selection are depleted for association with PD.
