...
Encephalisation and the evolution of complex human-specific traits are thought to have increased the susceptibility of Homo sapiens to disorders of the brain compared to their aged non-human primate counterparts. This is seen in Alzheimer’s and Parkinson’s disease, which have not been observed naturally on a pathological or phenotypic level in non-human species. Likewise, unique motor dysfunction in amyotrophic lateral sclerosis (ALS) supports the selective vulnerability of the highly-developed corticomotoneuronal system in man. Human-lineage-specific genomic sequences have been shown to be enriched for brain-specific elements and risk loci for neurodegenerative disorders. Thus, while positive natural selection has driven human adaptive evolution,8 it may be possible that the same selected alleles also influence risk of neurodegenerative disease. 

It is not known whether this neuro-vulnerability arose after divergence from other species or whether it represents a more recent phenomenon, characteristic of modern-day humans over other hominids. As anatomically-modern humans migrated out of Africa 50 to 100 thousand years ago, they interbred with archaic hominins including Neanderthals. As a result, Neanderthal DNA accounts for approximately 1–4% of the modern Eurasian genome. While most Neanderthal DNA experienced purifying selective pressures, positive selection of archaic alleles may have contributed to modern human adaptation to the non-African environment through modulating dermatological, immunological and metabolic function. On the other hand, these introgressed Neanderthal alleles have also been implicated in influencing disease risk such as in depression and skin disorders. It remains unclear how much Neanderthal admixture has affected our risk of neurodegenerative disorders. While Simonti and colleagues found that Neanderthal single nuclear polymorphisms (SNPs) may be associated with "neurological" phenotypes through electronic health records of European patients, these nervous system traits were not representative of neurodegenerative disease or common neurological disorders. More recently, one study aimed to address the complex nature of medically-relevant traits and the genome-wide influence of Neanderthal admixture using UK Biobank data. This found that introgressed variants were depleted for heritability of high-level cognitive traits. 
...
results_w_baseline <-
  ...
  dplyr::group_by(GWAS, cutoff, selection_metrics) %>% 
  dplyr::mutate(
    Z_score_FDR = 
      p.adjust(
        p = Z_score_P, 
...
- Using FDR-corrected results, a significant depletion was observed for:
    - AD $h^2_{SNP}$ using the top 2% CLR statistic. Given that this is only observed for the top 2% CLR statistic, and there is no nominal depletion (co-efficient p-value < 0.05) across either the top 0.5% or 1%, this does not suggest that SNPs under positive selection are depleted for association with AD, as one might conclude from the significant p-value.
    - PD $h^2_{SNP}$ using the top 0.5% LA. Further, there was a nominal depletion (co-efficient p-value < 0.05) of PD $h^2_{SNP}$ using the top 1% LA, suggesting that introgressed SNPs are depleted for association with PD.
    - PD $h^2_{SNP}$ using the top 0.5%, 1% and 2% XP-EEH statistic, suggesting that SNPs under positive selection are depleted for association with PD.
...
- While no annotations were significantly enriched/depleted when using the ALS GWAS, there were nominal results (co-efficient p-value < 0.05), including:
    - An enrichment ALS $h^2_{SNP}$ using the top 0.5%, 1% iHS statistic and the top 2% of the XP-EEH statistic. Notably, both statistics are variants on EEH and are thought to be complementary in terms of their scope: *"whereas iHS has more power to detect incomplete sweeps, XP-EHH is useful when the sweep is near fixation within one population"* (PMID: [24274750](https://pubmed.ncbi.nlm.nih.gov/24274750/)). 
    - A depletion of ALS $h^2_{SNP}$ using the top 0.5%, 1% and 2% CLR.
    - Confusingly, iHS, XP-EEH and CLR are all metrics of positive selection, thus an enrichment and depletion appears counter-intuitive. However, it is important to remember that these metrics detect particular selective processes that act on a particular timeframe. As stated in Pardiñas et al. 2018, iHS (and by extension XP-EEH) are *"better powered to detect recent selective sweeps"*, while CLR *"incorporates information about the spatial variability of genomic variability and corrects explicitly for evidence of background selection, thus being able to detect signals from 60,000 to 240,000 years ago"*. Thus, it is possible that SNPs from recent selective sweeps are enriched for association with ALS, while those SNPs from older selective sweeps are depleted for association with ALS.
...
- FDR multiple test correction was applied across each GWAS and Category, correcting for the number of annotations of interest run (i.e. 3 cut-offs x 5 selection metrics). This is comparable to the strategy applied for the annotations of interest, but is probably not sufficiently stringent, given the number of baseline annotations.
...
```{r}
print("Results averaged across 3 cut-offs x 5 selection metrics (FDR < 0.05):")
